...
public class StarRocksTable implements Table, SupportsWrite, SupportsRead {
    ...
    private PropertiesSettings addTableInfoForOptions(Map<String, String> options) {
        ...
        String tableIdent = properties.getProperty(STARROCKS_TABLE_IDENTIFIER);
        if (tableIdent == null && identifier != null) {
            properties.setProperty(STARROCKS_TABLE_IDENTIFIER, identifierFullName(identifier));
        } else if ( tableIdent!= null && identifier == null){
            String [] namespaces = new String[] {tableIdent.split("\\.")[0]};
            String tableName = tableIdent.split("\\.")[1];
            identifier = Identifier.of(namespaces, tableName);
        }
        return properties;
    }
}
